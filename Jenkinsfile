...
"""
  ...
  stages {
    ...
    stage('Deploy to staging') {
      ...
      steps {
        container('tools') {
          sh "git clone -b test https://$GIT_CREDS_USR:$GIT_CREDS_PSW@github.com/prachimittal2016/argocd-demo-deploy.git"
          ...
          dir("argocd-demo-deploy") {
            ...
            sh "cd ./staging && kustomize edit set image prachimittal2016/argocd-demo:${env.GIT_COMMIT}"
    ...
    stage('Deploy to Prod') {
      steps {
        ...
        container('tools') {
          dir("argocd-demo-deploy") {
            sh "cd ./prod && kustomize edit set image prachimittal2016/argocd-demo:${env.GIT_COMMIT}"
            sh "git commit -am 'Publish new version' && git push || echo 'no changes'"
          }
        }
      }
